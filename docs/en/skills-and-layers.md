...
Skills are filtered before entering the override tree, so non-matching candidates cannot affect conflict resolution and do not reach MCP, compiler, or ordinary CLI queries. The cross-project default asset boundary is `~/.skill-central/skills` plus `~/.skill-central/rules`; both directories are created on first startup. A repository containing `skill-central.yaml` is an explicit project-layer override rather than an accidental current-working-directory fallback.

Users may explicitly select one reusable asset-library root from **Sync → Local asset library** or **Personal settings → Asset library**. A valid root must contain both `skills/` and `rules/`; once selected, Board, CLI, and MCP use those two directories together. The validated choice is persisted in `~/.skill-central/settings.json`. **Use default directory** selects the initialized Home library and never deletes source files. The Registry checkout field on Sync is separate and only supplies synchronization plans. `SKILL_CENTRAL_ASSET_ROOT` is the controlled environment override for automation and packaged runtime handoff. Coding Agents can consume Rules through `rule://registry` and `rule://rule/<id>` resources, `rules:all` and `rule:<id>` prompts, or the `rules.list` and `rules.get` tools.
